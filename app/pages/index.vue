<template>
  <div v-if="user || isGuestMode" class="h-screen w-full flex flex-col bg-slate-50 overflow-hidden font-sans relative">
      ...
      <aside class="w-full flex-1 md:flex-none md:w-96 bg-white flex flex-col overflow-hidden shadow-inner md:shadow-xl z-10 relative">
        <div class="p-4 border-b bg-white flex-shrink-0">
          ...
          <div class="mt-2 space-y-2">
            ...
            <select v-model="selectedCategory" class="w-full bg-slate-100 border-none rounded-xl py-2.5 px-4 text-sm text-slate-600 outline-none focus:ring-2 focus:ring-green-500">
        ...
        <div class="flex-1 overflow-y-auto p-4 space-y-3 pb-20 md:pb-4 bg-slate-50/50">
          ...
          <template v-else>
            ...
            <div v-if="!filteredItems || filteredItems.length === 0" class="text-center text-slate-400 text-sm py-12">
    ...
    <ItemDetailsModal :item="selectedItem" :isLoggedIn="!!user" :distanceText="selectedItem ? formatDistance(selectedItem) : ''" @close="selectedItem = null" @trigger-login="selectedItem = null; isGuestMode = false;" />
